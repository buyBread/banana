...
using namespace ngl::shaders::sm_babyphatnormal;
...
bool ngl::shaders::sm_babyphatnormal::initialize() {
    if (!material_vertex_program.create({ e_shader_program::sm_babyphatnormal_material_vertex }))
        return false;

    for (size_t index = 0; index < pixel_programs.size(); ++index) {
        if (!pixel_programs[index].create({ e_shader_program::sm_babyphatnormal_material_pixel, (u16)index }))
            return false;
    }

    if (!depth_shadow_vertex_program.create({ e_shader_program::sm_babyphatnormal_depth_shadow_vertex }))
        return false;

    program_exports::sm_babyphatnormal::material_vertex_program
        .write(material_vertex_program.get());

    program_exports::sm_babyphatnormal::unshadowed_pixel_program
        .write(pixel_programs[(size_t)e_pixel_variant::unshadowed].get());

    program_exports::sm_babyphatnormal::shadow_1_pixel_program
        .write(pixel_programs[(size_t)e_pixel_variant::shadow_1].get());

    program_exports::sm_babyphatnormal::shadow_2_pixel_program
        .write(pixel_programs[(size_t)e_pixel_variant::shadow_2].get());

    program_exports::sm_babyphatnormal::depth_shadow_vertex_program
        .write(depth_shadow_vertex_program.get());

    return true;
}

IDirect3DVertexShader9* ngl::shaders::sm_babyphatnormal::get_vertex_program(e_vertex_variant variant) {
    switch (variant) {
...
IDirect3DPixelShader9* ngl::shaders::sm_babyphatnormal::get_pixel_program(e_pixel_variant variant) {
