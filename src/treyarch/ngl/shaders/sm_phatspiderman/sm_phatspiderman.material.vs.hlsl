...
float3 non_rigid_eye         : register(c225);
float  non_rigid_start       : register(c226);
float  non_rigid_end         : register(c227);
float3 fog_eye_to_local      : register(c228);
float3 fog_normal            : register(c229);
float3 fog_position_0        : register(c230);
float3 fog_position_1        : register(c231);
float2 fogs_per_meter        : register(c232);
float4 horizon_map_matrix_u  : register(c233);
float4 horizon_map_matrix_v  : register(c234);

struct vertex_input {
    float3 position      : POSITION0;
    float2 texcoord      : TEXCOORD0;
    float3 non_rigid_normal : COLOR0;
...
vertex_output main(vertex_input input) {
    ...
    float4 mesh_position = float4(input.position * (1.0 / 512.0), 1.0);
    ...
    output.texcoord.xy = input.texcoord * 0.001;
    output.texcoord.zw = float2(
        dot(homogeneous_position, horizon_map_matrix_u),
        dot(homogeneous_position, horizon_map_matrix_v));

    float3 decoded_non_rigid_normal = normalize(
        input.non_rigid_normal * 2.0 - 1.0);
    float non_rigid_present =
        input.non_rigid_normal.x +
        input.non_rigid_normal.y +
        input.non_rigid_normal.z != 0.0 ?
        1.0 : 0.0;
    float non_rigid_distance = distance(local_position.xyz, non_rigid_eye);
    float non_rigid_weight = non_rigid_present * saturate(
        (non_rigid_distance - non_rigid_start) /
        (non_rigid_end - non_rigid_start));
    float3 local_normal = normalize(lerp(
        input.normal,
        decoded_non_rigid_normal,
        non_rigid_weight));
    ...
    output.ambient_lo.a = min(
        ...
        fogs_per_meter.y);
    output.ambient_hi.a = non_rigid_weight;
