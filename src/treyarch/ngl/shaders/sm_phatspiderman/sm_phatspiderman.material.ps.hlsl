...
float4 uv_scale_offset   : register(c39);
float4 fresnel           : register(c40);
float  translucent_flag  : register(c41);
float4 rim_lighting      : register(c42);

#if NGL_SM_PHATSPIDERMAN_SHADOW_COUNT > 0
float4 shadow_distance          : register(c43);
float4 depth_fog_color          : register(c44);
float4 depth_fog_control        : register(c45);
float4 fog_color                : register(c46);
float  exposure                 : register(c47);
float3 horizon_sky_color        : register(c48);
float3 horizon_sky_direction    : register(c49);
float3 horizon_sun_color        : register(c50);
float3 horizon_sun_direction    : register(c51);
float2 horizon_sun_angles       : register(c52);
float3 horizon_bounce_direction : register(c53);
float2 horizon_bounce_angles    : register(c54);
float  horizon_bounce_ratio     : register(c55);
float3 horizon_night_color      : register(c56);
#else
...
#endif
#if NGL_SM_PHATSPIDERMAN_SHADOW_COUNT > 1
...
SamplerState normal_sampler            : register(s6);
SamplerState specular_exponent_sampler : register(s7);
SamplerState specularity_sampler       : register(s8);
SamplerState reflectivity_sampler      : register(s9);
...
#elif NGL_SM_PHATSPIDERMAN_SHADOW_COUNT == 2
...
half4 main(pixel_input input) : COLOR0 {
    ...
    float2 material_texcoord =
        input.texcoord.xy * uv_scale_offset.xy +
        uv_scale_offset.zw;

    half specularity = tex2D(
        specularity_sampler,
        material_texcoord).g;
    half reflectivity = tex2D(
        reflectivity_sampler,
        material_texcoord).g;
    half3 material_controls = saturate(
        half3(input.ambient_hi.a, specularity, reflectivity) *
    ...
    half4 diffuse = tex2D(diffuse_sampler, material_texcoord);
