...
float  diffuse_effect_1 : register(c60);
float4 decal_effect_0   : register(c61);
float  decal_effect_1   : register(c62);
...
#else
float4 diffuse_effect_0 : register(c50);
float  diffuse_effect_1 : register(c51);
float4 decal_effect_0   : register(c52);
float  decal_effect_1   : register(c53);
float4 normal_mad_0     : register(c54);
float4 normal_mad_1     : register(c55);
float2 alpha_mad_0      : register(c56);
float2 alpha_mad_1      : register(c57);
float2 alpha_mad_2      : register(c58);
#endif
...
float4 diffuse_effect_0 : register(c56);
float  diffuse_effect_1 : register(c57);
float4 decal_effect_0   : register(c58);
float  decal_effect_1   : register(c59);
float4 normal_mad_0     : register(c60);
float4 normal_mad_1     : register(c61);
float2 alpha_mad_0      : register(c62);
float2 alpha_mad_1      : register(c63);
float2 alpha_mad_2      : register(c64);
#else
float4 diffuse_effect_0 : register(c47);
float  diffuse_effect_1 : register(c48);
float4 decal_effect_0   : register(c49);
float  decal_effect_1   : register(c50);
float4 normal_mad_0     : register(c51);
float4 normal_mad_1     : register(c52);
float2 alpha_mad_0      : register(c53);
float2 alpha_mad_1      : register(c54);
float2 alpha_mad_2      : register(c55);
#endif
#endif
...
SamplerState decal_normal_sampler     : register(s8);
    #else
SamplerState diffuse_specular_sampler : register(s4);
SamplerState diffuse_normal_sampler   : register(s5);
SamplerState decal_alpha_sampler      : register(s6);
SamplerState decal_normal_sampler     : register(s7);
    #endif
#else
    #if NGL_SM_DECAL_GOBO
...
SamplerState decal_normal_sampler     : register(s7);
    #else
SamplerState diffuse_specular_sampler : register(s3);
SamplerState diffuse_normal_sampler   : register(s4);
SamplerState decal_alpha_sampler      : register(s5);
SamplerState decal_normal_sampler     : register(s6);
    #endif
#endif

struct pixel_input {
    float3 local_position      : TEXCOORD0;
    float4 shadow_position_0   : TEXCOORD1;
    float4 shadow_position_1   : TEXCOORD2;
    float4 texture_coordinates : TEXCOORD3;
#if NGL_SM_DECAL_GOBO
    float4 tangent_gobo_z      : TEXCOORD4;
    float4 binormal_gobo_y     : TEXCOORD5;
    float4 normal_gobo_x       : TEXCOORD6;
#else
    float3 tangent_gobo_z      : TEXCOORD4;
    float3 binormal_gobo_y     : TEXCOORD5;
    float3 normal_gobo_x       : TEXCOORD6;
#endif
    half4 ambient_lo           : TEXCOORD7;
    half4 ambient_hi           : TEXCOORD8;
#if NGL_SM_DECAL_HORIZON
    half2 horizon_coordinates  : TEXCOORD9;
#endif
};
...
local_light evaluate_local_light(int index, float3 position) {
    ...
    output.color = light_color[index] * output.attenuation;

    return output;
}

half compare_shadow(SamplerState shadow_map, float4 position) {
    float3 projected_position = position.xyz / position.w;
...
half4 main(pixel_input input) : COLOR0 {
    ...
    half2 tangent_normal = lerp(
        ...
        decal_normal,
        normal_blend);
    half normal_height = sqrt(
        1.0 - saturate(dot(tangent_normal, tangent_normal)));
    ...
    half effect_bias = saturate(
        material_effect_1 * special_effects_matrix[0].x +
        special_effects_matrix[1].x);
    ...
    half specular_power = saturate(
        material_effect_0.w * special_effects_matrix[0].z +
        special_effects_matrix[1].z) * 64.0;
    ...
    half3 reflected_color = 0.0;
    half3 direct_lighting = 0.0;
    half3 rim_source = 0.0;
    ...
    lights[0].color = light_color[0] * min(
        lights[0].attenuation,
        shadow_visibility);
    ...
    for (int index = 0; index < 4; ++index) {
        local_light light = evaluate_local_light(index, input.local_position);

        if (index == 0) {
            light.color = light_color[index] * min(
                light.attenuation,
                shadow_visibility);
        }
        ...
        reflected_color +=
            light.color * evaluate_specular(
                reflection_direction,
                light,
                specular_power);
        rim_source += light.color;
    }
#endif
    ...
    half4 diffuse_specular_sample = tex2D(
    ...
    half3 material_controls = saturate(
        material_effect_0.xyz * special_effects_matrix[2].xyz +
...
#if NGL_SM_DECAL_HORIZON
    lighting += horizon_sun + sky_bounce;
    lighting.b +=
