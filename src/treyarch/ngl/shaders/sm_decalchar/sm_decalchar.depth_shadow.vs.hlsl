...
row_major float4x4 screen_matrix : register(c192);
row_major float4x4 local_matrix  : register(c196);
row_major float4x4 texture_coordinate_matrix : register(c200);
...
vertex_output main(vertex_input input) {
    ...
    output.position = mul(float4(local_position.xyz, 1.0), screen_matrix);
    output.texture_coordinates = mul(
        float4(input.texcoord, 0.0, 1.0),
        texture_coordinate_matrix).xy;
