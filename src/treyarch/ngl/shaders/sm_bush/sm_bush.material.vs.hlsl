...
row_major float4x4 local_matrix                  : register(c24);
row_major float4x4 light_world_view_projection_0 : register(c28);
row_major float4x4 light_world_view_projection_1 : register(c32);
...
float4 movement             : register(c44);
float4 time                 : register(c45);
...
struct vertex_output {
    ...
    float2 horizon_texcoord  : TEXCOORD9;
};

vertex_output main(vertex_input input) {
    vertex_output output;

    float phase = time.x * input.color.a * movement.z * 0.1;
    float3 displaced_position = input.position +
        sin(movement.x * input.position + phase) * movement.y;
    float4 local_position = mul(float4(displaced_position, 1.0), local_matrix);

    output.position          = mul(float4(displaced_position, 1.0), screen_matrix);
    ...
    output.texcoord = float4(input.texcoord, 0.0, 1.0);

    float3 gobo_position = mul(local_position, local_to_gobo).xyz;
    output.tangent_gobo_z  = float4(input.tangent, gobo_position.z);
    output.binormal_gobo_y = float4(
        normalize(cross(input.tangent, input.normal)),
        gobo_position.y);
    ...
    output.horizon_texcoord = float2(
