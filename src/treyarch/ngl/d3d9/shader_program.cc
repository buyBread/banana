...
bool ngl::d3d9::vertex_program::create(shaders::shader_key key) {
    ...
    if (!shaders::describe_shader(key, description) ||
        description.stage != shaders::e_shader_stage::vertex)
        return false;
    ...
    HRESULT result = references::device.read()->CreateVertexShader(
        (const DWORD*)bytecode.data(),
        &shader_);
...
bool ngl::d3d9::pixel_program::create(shaders::shader_key key) {
    ...
    HRESULT result = references::device.read()->CreatePixelShader
